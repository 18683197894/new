...
<div class="x-body house_add" style="display:none;position:relative;z-index:20">
    <form class="layui-form" id="myform" action="{{ url('/backend/carousel-add_web') }}" enctype="multipart/form-data" method="post"lay-filter="component-form-group" au'tocomplete="off">'
      	@csrf
...
<div class="x-body link_edit" style="display:none;position:relative;z-index:20">
    <form class="layui-form" id="edit" action="{{ url('/backend/carousel-edit_web') }}" enctype="multipart/form-data" method="post"lay-filter="edit" au'tocomplete="off">'
        @csrf
  ...
  layui.use(['form','upload'],function(){
    ...
    function houseDelAll()
    {
      ...
      layer.confirm('确定要删除ID为 '+ids+' 的轮播图吗?',function(index){
        $.ajax({
          'url' : "{{ url('/backend/carousel-del_web') }}",
          'type' : 'post',
