...
@endsection
@section('open')
    <div class="x-body menu_add" style="display:none;position:relative;z-index:20">
        <form action="{{ url('/backend/sys/menu_add') }}" method="post" class="layui-form layui-form-pane">
                @csrf
                ...
                <div class="layui-form-item">
                    ...
                    <div class="hint-block">详情请参考：<a class="aurl" href="{{ url('/backend/lcon') }}" target="_blank">{{ url('/backend/lcon') }}</a></div>
                </div>
                <div class="layui-form-item">
                    <label for="name" class="layui-form-label">
                        排序
    ...
    <div class="x-body menu_edit" style="display:none;position:relative;z-index:20">
        <form action="{{ url('/backend/sys/menu_edit') }}" method="post" class="layui-form layui-form-pane">
                @csrf
...
@section('content')
    ...
    <div class="x-body">
      <xblock>
        <button class="layui-btn" onclick="menu_add('创建菜单')"><i class="layui-icon"></i>创建菜单</button>
      ...
      <table class="layui-table"  frame="hsides">
        ...
        <tbody>
          ...
          <tr id="{{ $val['id'] }}" class="0">
            ...
            <td><span class="kuaiji">{{ $val['title'] }}</span>  <span class="quan" onClick="menus_add('{{ $val['id'] }}','{{ $val['title'] }}')">+</span></td>
            <td>{{ $val['url'] }}</td>
            <td lcon="{{ $val['lcon'] }}"><i class="iconfont">@php echo $val['lcon'] @endphp</i></td>
          ...
          <tr class="{{ $value['pid'] }}">
            ...
            <td>{{ $value['url'] }}</td>
            <td lcon="{{ $value['lcon'] }}"><i class="iconfont">@php echo $value['lcon'] @endphp</i></td>
...
<script>
        ...
        function menu_stop(obj,id)
        {
          ...
          layer.confirm('确定要'+itps+'吗?',function(index){
            $.ajax({
              'url':'{{ url("/backend/sys/menu_status") }}',
        ...
        function menu_edit(obj,id,title,name)
        {
          ...
          $('.menu_edit input[name="lcon"]').val(tr.find('td').eq(3).attr('lcon'));
          $('.menu_edit input[name="urls"]').val(tr.find('td').eq(2).html());
          $('.menu_edit input[name="sort"]').val(tr.find('td').eq(4).find('input').val());
          var width = ($(window).width() * 0.5)+'px';
          var height = ($(window).height() * 0.8)+'px';
          layer.open({
            type : 1,
