...
<div class="layui-card dwg_upload" style="display:none">
    ...
    <div class="x-body key_add" style="display:none;position:relative;z-index:20">
        <form action="{{ url('backend/sys/front-keyword-add') }}" method="post" class="layui-form layui-form-pane">
                @csrf
      ...
      <table class="layui-table">
        <thead>
          <tr>
            ...
            <th style="width:11%">路由</th>
            <th style="width:10%">网页标题</th>
            <th style="width:18%">网页关键字</th>
            <th style="width:18%">网页描述</th>
            <th style="width:15%">banner图</th>
        ...
        @foreach($keyword as $v)
          <tr>
            ...
            <td>
              @if($v->image)
               <a class='layui-btn layui-btn-xs layui-btn-normal tu' id="{{ $v->id }}"><i class='layui-icon'></i>上传图片</a>
                <a target="_blank" href="{{ asset($v->image) }}" class="layui-btn layui-btn-xs  layui-btn-normal">查看</a>
                <a class="layui-btn layui-btn-xs  layui-btn-danger" onclick="banner_del(this,'{{ $v->id }}')">删除</a>
              @else
                <a class='layui-btn layui-btn-xs layui-btn-normal tu' id="{{ $v->id }}"><i class='layui-icon'></i>上传图片</a>
              @endif
...
$('.tu').on('click',function(){
    var id = $(this).attr('id');
    var re_data = "{data:{id:"+id+",_token:'"+$("meta[name='csrf-token']").attr('content')+"'}}";
...
layui.use(['upload'], function () {
    ...
    upload.render({
      elem: '#dwg_upload'
      ,url: '{{ url("/backend/sys/front-keyword-image") }}'
      ,method : 'post'
      ,field:'file'
      ,accept: 'image' //普通文件
      ,exts :'jpg|png|jpeg'
	...
	{	
		...
		layui.use(['form','jquery'],function(){
          ...
          form.on('submit(key_edit)',function(data){
          		$.ajax({
          			url : "{{ url('/backend/sys/front-keyword-edit') }}",
          			type : 'post',
      ...
      function key_del(obj,id){
          ...
          layer.confirm('确定要删除吗？',function(index){
              //发异步删除数据
              $.ajax({
                url : "{{ url('/backend/sys/front-keyword-del') }}",
                type : 'post',
      ...
      function banner_del(obj,id)
      {
          layer.confirm('确定要删除吗？',function(index){
              //发异步删除数据
              $.ajax({
                url : "{{ url('/backend/sys/front-keyword-del-banner') }}",
                type : 'post',
