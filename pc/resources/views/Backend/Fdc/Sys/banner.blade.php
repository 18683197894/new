...
<div class="x-body house_add" style="display:none;position:relative;z-index:20">
    <form class="layui-form" id="myform" action="{{ url('/backend/fdc/banner-add') }}" enctype="multipart/form-data" method="post"lay-filter="component-form-group" au'tocomplete="off">
      	@csrf
    <div class="layui-form-item">
      <label class="layui-form-label">页面</label>
      <div class="layui-input-block">
        <select name="key" lay-verify="required" lay-filter="aihao">
          <option value=""></option>
          @foreach($page as $k => $v)
...
<div class="x-body link_edit" style="display:none;position:relative;z-index:20">
    <form class="layui-form" id="edit" action="{{ url('/backend/fdc/banner-edit') }}" enctype="multipart/form-data" method="post"lay-filter="edit" au'tocomplete="off">
        @csrf
      ...
      <table class="layui-table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>{{ $v->id }}</td>
            <td>{{ isset($page[$v->key])?$page[$v->key]:'' }}</td>
            <td>{{ $v->url }}</td>
            <td style="padding:8px 8px"><a target="_blank" href="@if(!empty($v->image)) {{ url($v->image) }} @endif"><img style="width:100%;height:100%;max-width:150px;max-height:70px" src="@if(!empty($v->image)) {{ asset($v->image) }} @endif"></a></td>
            <td>{{ $v->stort }}</td>
            <td>
...
<script type="text/javascript">
  layui.use(['form','upload'],function(){
    ...
    function houseDelAll()
    {
      ...
      layer.confirm('确定要删除ID为 '+ids+' 的轮播图吗?',function(index){
        $.ajax({
          'url' : "{{ url('/backend/fdc/banner-del') }}",
          'type' : 'post',
  ...
  { 
    ...
    var height = ($(document).height() * 0.55)+'px';
    layer.open({
      type : 1,
      title : title,
      fix: false, //不固定
